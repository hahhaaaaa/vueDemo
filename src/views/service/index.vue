<template>
  <div>
    ...
    <div class="el-dialog__wrapper" id="photoBlock" style="display: none">
      <img
        @click="closeCarousel"
        src="https://p0.meituan.net/education/1bcf4b282ab1721887398c164c76f65a385.png"
        alt="删除按钮"
        style="position: relative;right: -400px;top: 100px;"
      >
    ...
    <el-table :data="rows">
        ...
        <template slot-scope="scope" prop="photoWall1">
          <div style="position: relative;">
            ...
            >
              <span
                class="photoWall1Bg"
                @click="openCarousel(scope.$index,scope.row)"
                style="color:#fff;font-size: 30px;line-height: 13px; cursor: pointer;"
              >...</span>
            </div>
      ...
      <el-table-column prop="species" label="服务种类"  width="80" :show-overflow-tooltip="true"></el-table-column>
      <el-table-column prop="plan" label="时间段" width="80" :show-overflow-tooltip="true"></el-table-column>
      <el-table-column prop="weight" label="宠物的大小" width="100"></el-table-column>
      <el-table-column prop="service" label="服务规格：普修，精修" width="100"></el-table-column>
      <el-table-column prop="time" label="耗时：正常耗时" width="80"></el-table-column>
      <el-table-column prop="waiter" label="服务员等级：普通，高级" width="100"></el-table-column>
      <el-table-column prop="price" label="价格：基准价格" width="120"></el-table-column>
      <el-table-column label="操作" width="120">
        <template slot-scope="scope">
          ...
          <el-button
            size="mini"
            type="danger"
            @click="handleDelete(scope.$index, scope.row,'upDate')"
          >删除</el-button>
        </template>
      </el-table-column>
      <el-table-column label="上，下架（默认激活蓝色，已上架）" width="120">
        <template slot-scope="scope">
    ...
    <el-dialog title="服务信息" :visible.sync="dialogFormVisible" :before-close="handleClose">
      <el-form :model="form">
        <el-form-item label="选择店铺" prop="store">
          <el-select v-model="form.store" placeholder="请选择要添加的店铺">
            <el-option
...
export default {
  data() {
    return {
      ...
      form: {
        ...
        name: "", //服务类别
        img:
          "https://p1.meituan.net/dpdeal/761f1769fdc11d2fd906200062c374e4164109.jpg%40450w_280h_1e_1c_1l%7Cwatermark%3D0", //图片路径
        photoWall1:
          "https://p1.meituan.net/dpdeal/9fb2aaadf8bf005361b73c85573e479a144425.jpg%40450w_280h_1e_1c_1l%7Cwatermark%3D0", //相册
        photoWall2:
          "https://p0.meituan.net/dpdeal/0727a48174e61b6d12800bcef78f51a0405130.jpg%40450w_280h_1e_1c_1l%7Cwatermark%3D0", //相册
        photoWall3:
          "https://p1.meituan.net/dpdeal/b55c9a45f38903481abc69a2a2454ac0234731.jpg%40450w_280h_1e_1c_1l%7Cwatermark%3D0", //相册
        photoWall4:
          "https://p1.meituan.net/dpdeal/2a483d24a0ca5b10816acfc1c0c19d611057600.jpg%40450w_280h_1e_1c_1l%7Cwatermark%3D0", //相册
  ...
  mounted() {
    //渲染完成后自动调用该函数
    const userId = JSON.parse(localStorage.user)[0]._id;
  ...
  methods: {
    ...
    openCarousel(index, row) {
      ...
      document.getElementById("photoBlock").style.cssText =
        "width: 100%;height: 100%;background: #000;opacity: .8;position: fixed;z-index: 99;";
    },
    closeCarousel() {
      //关闭走马灯
    ...
    handleUpDatePet(index, row) {
      //打开表单，将请求的数据放到data里
      console.log(row.store);
      
      this.dialogFormVisible = true;
      this.form.store = row.store._id;
